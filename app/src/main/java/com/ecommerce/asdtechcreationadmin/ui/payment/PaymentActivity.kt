...
import com.ecommerce.asdtechcreationadmin.data.model.PaymentsResponse
import com.ecommerce.asdtechcreationadmin.databinding.ActivityPaymentBinding
import com.ecommerce.asdtechcreationadmin.ui.adapter.PaymentAdapter
...
class PaymentActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.fabAddPayment.setOnClickListener {
            addPaymentLauncher.launch(
                Intent(this, AddPaymentActivity::class.java)
            )
        }
    ...
    private fun loadPayments() {
        ...
        binding.progressPayments.visibility = View.VISIBLE
        binding.emptyState.visibility = View.GONE

        ApiClient.apiService.getPayments().enqueue(object : Callback<PaymentsResponse> {
            ...
            override fun onResponse(
                ...
                binding.progressPayments.visibility = View.GONE

                if (response.isSuccessful && response.body()?.status == "success") {
                    allPayments = response.body()?.data ?: emptyList()
                    binding.txtTotalPaymentsCount.text = allPayments.size.toString()
                    applyFilter()
                } else {
                    allPayments = emptyList()
                    binding.txtTotalPaymentsCount.text = "0"
                    applyFilter()
    ...
    private fun applyFilter() {
        ...
        if (searchQuery.isNotEmpty()) {
            filtered = filtered.filter {
                it.receipt_number.contains(searchQuery, ignoreCase = true) ||
                        (it.client_name?.contains(searchQuery, ignoreCase = true) == true) ||
                        (it.company_name?.contains(searchQuery, ignoreCase = true) == true) ||
                        (it.invoice_number?.contains(searchQuery, ignoreCase = true) == true)
            }
        }

        filteredPayments = filtered

        val totalPages = maxOf(1, ceil(filteredPayments.size / pageSize.toDouble()).toInt())
    ...
    private fun renderPage() {
        ...
        else filteredPayments.subList(startIndex, endIndex)
